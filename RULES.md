# Project Rules — Sri Guru Granth Sahib Ji Reader

These are the binding conventions for every future change. Follow them so the
app keeps working across SGPC year releases, deploys, and offline installs.
`README.md` describes *what the app does*; this file describes *how to work
...
- Commit messages follow the repo's imperative style
  ...
  churn (dev↔build path flip), `.next/`, or secrets. Verify with
  `git status`, `git diff`, `git log --oneline -10` before committing, and
